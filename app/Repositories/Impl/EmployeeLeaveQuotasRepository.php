    ...
    public function employeeLeaveQuotas($data)
    {
        try {
            ...
            $leaveQuotas = DB::table('emp_leave_type AS t')
                ...
                ->select('t.id', 't.leave_type_name', DB::raw('SUM(CASE WHEN l.leave_type_id IS NOT NULL THEN l.leave_day ELSE 0 END) AS use_leave_days'), 'e.quota AS all_quota')
                ->groupBy('t.id', 't.leave_type_name', 'e.quota')
                ->orderBy('t.id', 'ASC')
                ->get();
